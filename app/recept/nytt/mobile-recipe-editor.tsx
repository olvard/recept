...
export function MobileRecipeEditor() {
  ...
  if (!id && flow === "choice") return <div className="page-wrap editor-page"><NewRecipeChoice onCreate={() => setFlow("create")} onImport={() => setFlow("import")} onBack={close} /></div>;
  if (!id && flow === "import") return <div className="page-wrap editor-page"><RecipeImportForm onBack={() => setFlow("choice")} onImported={(draft, review) => { setInitialDraft(draft); setImportReview(review); setFlow("create"); }} /></div>;
  return <div className="page-wrap editor-page"><RecipeEditorForm mode={recipe ? "edit" : "create"} recipe={recipe} initialDraft={initialDraft} importReview={importReview} onClose={close} onSaved={(saved) => router.push(`/recipes/${saved.id}`)} className="mobile-editor" /></div>;
}
