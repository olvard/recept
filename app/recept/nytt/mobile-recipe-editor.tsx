...
export function MobileRecipeEditor() {
  ...
  if (!isHydrated) return <div className="page-wrap loading-state"><h1 className="sr-only">Nytt recept</h1><p className="eyebrow">Nytt recept</p><p>Laddar recept…</p></div>;
  if (id && !loaded) return <div className="page-wrap loading-state"><h1 className="sr-only">Nytt recept</h1><p>Laddar receptet…</p></div>;
  if (id && !recipe) return <div className="page-wrap editor-page"><p className="eyebrow">Receptet saknas</p><h1>Hittar inte receptet</h1><p>Receptet finns inte längre bland recepten.</p><button className="button" type="button" onClick={close}>Tillbaka</button></div>;
