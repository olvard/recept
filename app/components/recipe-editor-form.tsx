...
import { validateRecipeDraft, type CanonicalRecipe, type RecipeDraft } from "@/lib/recipe-vault";
...
type RecipeEditorFormProps = {
  ...
  onClose: () => void;
  onSaved: (recipe: CanonicalRecipe) => void;
  containerRef?: React.RefObject<HTMLElement | null>;
  className?: string;
  dialog?: boolean;
};

export function RecipeEditorForm({ mode, recipe, initialDraft, importReview, onClose, onSaved, containerRef, className = "", dialog = false }: RecipeEditorFormProps) {
  ...
  const shouldPersistOnUnmount = useRef(true);
  ...
  useEffect(() => { if (timer.current) clearTimeout(timer.current); timer.current = setTimeout(() => commitDraft(draft), 500); return () => { if (timer.current) clearTimeout(timer.current); }; }, [draft]); // eslint-disable-line react-hooks/exhaustive-deps

  const update = (changes: Partial<RecipeDraft>) => {
  ...
  const discardDraft = () => {
    if (!clearDraft(key)) return;
    shouldPersistOnUnmount.current = false;
    onClose();
  };
