...
import { requestJson } from "@/lib/editor-client";
...
export function RecipeVaultProvider({ children }: { children: React.ReactNode }) {
  ...
  const mutate = useCallback(async (url: string, method: string, draft?: RecipeDraft) => { try { const result = await requestJson<{ recipe: CanonicalRecipe; index: RecipeSummary[] }>(url, { method, body: draft ? JSON.stringify(draft) : undefined }); setRecipes(result.index); setStorageError(null); return result.recipe; } catch (error) { setStorageError(error instanceof Error ? error.message : "Receptet kunde inte sparas."); return null; } }, []);
