...
export function VaultCatalog() {
  ...
  return <div className="page-wrap vault-page">
    ...
    {visible.length ? <><section className="recipe-grid" aria-label="Receptposter">{visible.map((recipe) => <article className="recipe-card" key={recipe.id}>
      <p className="archive-line">#{recipe.id} · {recipe.category} · {recipe.prepMinutes} min</p><h2>{recipe.title}</h2><p className="note">{recipe.note}</p><div className="card-context"><p className="context">{recipe.context}</p><Link className="open-link" href={`/recipes/${recipe.id}`}>Öppna <span aria-hidden="true">→</span></Link></div>
    </article>)}</section>
